...
| `course-init-prompt` | Run after `course-split-section` (a separate step, not auto-invoked). Drafts the Chinese `init_prompt.md`, folding in the user's durable teaching-style preferences (see `skills/course-init-prompt/references/teaching-style.md`): learn-from-explanation, motivation-first, efficient≠skipping, past papers woven into the matching round rather than piled at the end, and claude.ai LaTeX-rendering rules. Has the session connect new material to any attached prior-learning notes/cheatsheets, give **direct inline-quoted references** to any attached past-paper problems (tagged by family) at the matching teaching moment, and runs `writing:humanizer-zh` if the `writing` plugin is enabled in the project. The prompt length is whatever states the demands accurately — there is no line cap. |
