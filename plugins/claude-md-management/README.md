...
> Forked from [`anthropics/claude-plugins-official`](https://github.com/anthropics/claude-plugins-official). The only behavior change is that the `claude-md-improver` skill now applies its proposed updates directly instead of pausing for approval. Diffs and reasons are still shown inline; revert via `git` if any change is unwanted.
