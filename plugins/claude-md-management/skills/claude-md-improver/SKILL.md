...
**This skill writes to CLAUDE.md files directly.** It presents a quality report, shows each targeted change with a diff and reason, and applies the updates immediately — no approval prompt. The user can revert via git if anything was unwanted.
...
After outputting the quality report, apply targeted updates directly — do **not** ask for confirmation. For each change, show the diff and reason inline with the application (Phase 5 below) so the user can see what changed and revert via git if needed.
